...
export default function Upload() { // 컴포넌트의 명은 직관적으로 설정(업로드하는 컴포넌트입니당!)
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      // if/else와 try/catch의 차이에 대해 궁금하죠? 내가 예상할 수 있는 부분은 if/else로 처리하고 네트워크로 인한 문제 혹은 파일읽는데 생기는 예외처리를 catch를 통해 처리한다고 생각하면 되요!!
      if (event.target.files && event.target.files.length > 0) {
        ...
        if (extension && allowedExtensions.includes(extension)) {
          ...
          alert("unexpected error are incured");
        }
      }
    } catch (error) {
      console.error("Error handling file change:", error.message);
      alert("An error occurred while handling the file change.");
    }
  }
